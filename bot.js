...
var xPromptIdx=0;
async function buildXCaption(){var p=xPrompts[xPromptIdx%xPrompts.length];xPromptIdx++;var ai=await smartAsk(systemPrompt(false),p);ai=ai.split(' ').filter(function(w){return w.indexOf('http')!==0;}).join(' ').trim();return ai||TICKER+' on X';}
var socialsIdx=0;
...
app.listen(PORT,async function(){console.log('$Mpc bot on port '+PORT);try{await new Promise(function(r){setTimeout(r,2000);});}catch(_){}try{await registerWebhook();}catch(e){console.log(e.message);}try{resetSilence();}catch(_){}try{scheduleNextShoutout();}catch(_){}setInterval(function(){if(WEBHOOK_URL)try{fetch(WEBHOOK_URL+'/health').catch(function(){});}catch(_){}},4*60*1000);console.log('$Mpc bot live');});